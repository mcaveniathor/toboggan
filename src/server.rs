...
impl Database for DbServer {
    ...
    fn tree_names(self,_context: context::Context) -> Self::TreeNamesFut {
    ...
    fn new_tree(self,_context: context::Context, name:String) -> Self::NewTreeFut {
        match self.db.open_tree(&name) {
            Ok(_) => { info!("Created or opened tree successfully."); future::ready(Ok(())) },
            Err(e) => future::ready(Err(e.into()))
        }
    }
    type GenerateIdFut = Ready<Result<u64, Error>>;
    #[instrument(skip(self))]
    fn generate_id(self,_context: context::Context,) -> Self::GenerateIdFut {
        future::ready(self.db.generate_id().map_err(|e| e.into()))
    }
    type InsertFut = Ready<Result<Option<Vec<u8>>, Error>>;
    #[instrument(skip(self))]
    fn insert(self,_context: context::Context,tree: Option<String>, key:Vec<u8>, value: Vec<u8>) -> Self::InsertFut {
    ...
    #[instrument(skip(self))]
    fn get(self,_context: context::Context,tree:Option<String> ,key:Vec<u8>) -> Self::GetFut {
        let tree_ref: &sled::Tree;
        let tree_owned: sled::Tree;
        match tree {
            Some(name) => {
    ...
    fn remove(self, _context: context::Context,  tree: Option<String>, key: Vec<u8>,) -> Self::RemoveFut {
        ...
        future::ready(
            match tree_ref.remove(&key) {
                Ok(opt) => {
                    match opt {
                        Some(v) => {
                            info!("Removed key. The key previously had a value." );
                            Ok(Some(v.to_vec()))
                        },
                        None => { info!("Removed key. There was no previous value."); Ok(None) }
                    }
                }
                Err(e) => Err(e.into())
            }
